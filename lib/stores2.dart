...
class StorePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final WebButton = storeProvider.when(
      ...
      error: (e, s) => Text('エラー $e'),
      data: (state) {
        if (state.StoreWeb != null && state.StoreWeb.isNotEmpty) {
    ...
    final TabelogButton = storeProvider.when(
      ...
      error: (e, s) => Text('エラー $e'),
      data: (state) {
        if (state.StoreTabelog != null && state.StoreTabelog.isNotEmpty) {
          return ElevatedButton(
            style: ElevatedButton.styleFrom(
              fixedSize: Size(screenSize.width * 0.8, screenSize.height * 0.01),
    ...
    final TwitterButton = storeProvider.when(
      ...
      error: (e, s) => Text('エラー $e'),
      data: (state) {
        if (state.StoreTwitter != null && state.StoreTwitter.isNotEmpty) {
          return ElevatedButton(
            style: ElevatedButton.styleFrom(
              fixedSize: Size(screenSize.width * 0.8, screenSize.height * 0.01),
    ...
    final InstaButton = storeProvider.when(
      ...
      error: (e, s) => Text('エラー $e'),
      data: (state) {
        if (state.StoreInsta != null && state.StoreInsta.isNotEmpty) {
          return ElevatedButton(
            style: ElevatedButton.styleFrom(
              fixedSize: Size(screenSize.width * 0.8, screenSize.height * 0.01),
